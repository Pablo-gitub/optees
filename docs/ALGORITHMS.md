...
1. Nonlinear Programming: unconstrained and bound-constrained continuous
   optimization.
2. Graph Theory: shortest path first, then graph optimization methods.
3. Heuristics & Metaheuristics: TSP constructive/local-search baseline, then
   Simulated Annealing and problem-specific metaheuristics.
4. Scheduling and Robust/Stochastic Optimization: workflow-specific models
   built first on the MILP, Graph, and Heuristic foundations.
5. AI & Machine Learning: educational models and optimization links after the
