...
- status: "Optimal" | "Infeasible" | "Unbounded" | "NotSolved"
- objective: float | None (None unless Optimal)
- x_dict: {var_name: value} | {} (values only if Optimal)
...
- time_limit (seconds): soft limit passed to the backend. CP-SAT stops
  at the limit and may return "NotSolved" (we do not expose FEASIBLE).
...
def solve_milp(
    ...
    objective : float | None
        Optimal objective value if status == "Optimal"; else None.
    x_dict : dict[str, float]
        Variable values if status == "Optimal"; else {}.
    extras : dict[str, Any]
...
def _solve_mip_cbc_mixed(
    ...
    extras = {
        ...
        "nodes": getattr(solver, "nodes", None),  # may be None in some builds
    }
...
def _solve_model(
    model: cp_model.CpModel, *, time_limit: Optional[float]
...
def _pack_result(
    ...
    Notes
    -----
    - We only report "Optimal" and "Infeasible" explicitly.
      Any other CP-SAT status (e.g., FEASIBLE, UNKNOWN, MODEL_INVALID)
      is returned as "NotSolved".
    """
